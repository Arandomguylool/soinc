...
class PreloadingState extends MusicBeatState
{
	...
	function caching()
	{
		...
		var imagesPaths = [];
		var daimagePath = list.filter(text -> text.contains('assets/images'));
		var daSharedimagePath = list.filter(text -> text.contains('assets/shared/images'));
		var daiconsPath = list.filter(text -> text.contains('assets/images/icons'));
		var daCharactersPath = list.filter(text -> text.contains('assets/images/characters'));
		var daWeekPath = list.filter(text -> text.contains('assets/week' + PlayState.storyWeek + '/images'));
		
		#if cpp
		if (FlxG.save.data.preloadCharacters)
		{
			trace("caching images...");
			for (i in daimagePath)
			{
				//trace(i);
				if (!i.endsWith(".png"))
					continue;
