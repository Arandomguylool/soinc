	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
		          ...
		          case 'rainbow' | 'rme' | 'icu' | 'tol':
		          {
		            ...
		            var blue:BGSprite = new BGSprite("blue", -13, -35, 0.3, 0.3);
		            add(blue);
		            
		            var red:BGSprite = new BGSprite("red", -40, -62, 0.5, 0.5);
		            add(red);
		            
		            var green:BGSprite = new BGSprite("green", 48.5, 464.4, 0.7, 0.7);
		            add(green);
		...
		if (generatedMusic && PlayState.SONG.notes[Std.int(curStep / 16)] != null)
		{
			...
			if (/*camFollow.x != dad.getMidpoint().x + 150 && */!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && (!pointAtGF && !camPointEventEnabled))
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'soinc':
						defaultCamZoom = 1;
						camFollowX = dad.getMidpoint().x + 30;
						camFollowY = dad.getMidpoint().y + 100;
					case 'senpai':
			...
			if (PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection/* && camFollow.x != boyfriend.getMidpoint().x - 100*/ && (!pointAtGF && !camPointEventEnabled))
			{
				...
				switch (curStage)
				{
					...
					case 'rainbow':
						defaultCamZoom = 1.2;
						camFollowX = boyfriend.getMidpoint().x - 150; // x negativo = direita | x positivo = esquerda
						camFollowY = boyfriend.getMidpoint().y + 30; // y negativo = cima | y positivo = baixo
					case 'mall':
